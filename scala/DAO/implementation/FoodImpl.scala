...
import DAO.interface.FoodInterface
...
class FoodImpl extends FoodInterface{
    ...
    try {
      ...
      while (res.next()) {
        val v = new Food(res.getLong("id"), res.getString("name"), res.getString("measurement"), res.getDouble("calories_multiplier"), res.getDouble("proteins_multiplier"),
          res.getDouble("fats_multiplier"), res.getDouble("carbohydrates_multiplier"), res.getDouble("fibers_multiplier"))
        seq = seq :+ v
      }
      seq
    } catch {
  ...
  override def findAllUnique(): Seq[Food] = {
    var conn: java.sql.Connection = null
    try {
      conn = DatabaseConnection.getConnection()
      val stmt = conn.prepareStatement("SELECT DISTINCT name FROM food")
      val res = stmt.executeQuery()
      var seq: Seq[Food] = null
      while (res.next()) {
        val v = new Food(res.getLong("id"), res.getString("name"), res.getString("measurement"), res.getDouble("calories_multiplier"), res.getDouble("proteins_multiplier"),
